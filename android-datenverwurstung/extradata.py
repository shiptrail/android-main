...
re_base = "^(?P<date>\d{2}\.\d{2}\.\d{4}) (?P<time>\d{2}:\d{2}:\d{2}):\d{3} GMT\+\d{2}:\d{2}: "
re_battperc = re.compile(re_base+"Battery Percent: (?P<battperc>[\d\.]{4})")
...
re_network_type = re.compile(re_base+"Network Type Information: (?P<networktype>[A-za-z / \+0-9]*)")
# re_network_url = re.compile(re_base+"Network: Url: (?P<networkurl>[A-za-z:.\- / \+0-9]*)")
re_network_uuid = re.compile(re_base+"Network: Url: https://shiptrail.lenucksi.eu/v2/(?P<networkuuid>[a-z-0-9]*)/send")
...
re_network_responsecode = re.compile(re_base+"Network: Responsecode / Error: (?P<networkresponsecode>[0-9]*)")
...
def check_and_finish(event):
    ...
    if set_elems.issubset(set(event.keys())):
        print('current event done: {}'.format(event['timestamp']))
